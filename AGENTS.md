...
- 编辑器与实际游戏的场景、运行树、属性、输入、截图和错误诊断优先使用项目插件 `addons/godot_mcp` 对应的 Godot MCP/CLI；确定性批处理、超时/进程树管理和发布验收仍使用仓库受控包装器。MCP 输入的 `sent: true` 不是行为证据，必须以状态或截图确认；存在结构化命令时不得优先使用 `runtime eval`。
...
- 当前产品实现顺序以 `miko / ofuda_trace（巫女 A）+ hard` 六关参考路线为最高优先级。第二关 M2 可玩性修复完成后，M3–M5 先完成该路线；六关连续 Hard 1CC 通过前，不做 Normal/其余机体的全面平衡或全量矩阵。
- 六机体 Boss TTK 的 ±15% 不再是硬门槛。其余机体后续要求机制有效、可通关、无明显失效或极端拖时，允许因风险与功能差异形成有意义的击破时间差。
- 自动/无敌回放、控制器无错误、热力图或性能绿灯只作为工程诊断，不能证明可玩。关卡可玩门禁必须使用正常判定与实际输入：允许 Miss/Bomb，不允许 Continue、无敌、跳阶段、传送或自动避弹。
- “可通关”只是最低门槛，不能证明弹幕设计合格。M2 及 M3–M5 的敌群必须具备可辨识的 `入场 → 预告 → 发射 → 收束/退场` 动作，弹幕必须有明确的弹种职责、至少三个可读速度层级、可追溯的可见发射源和逐段发展的主母题；不得把不同内容原语压回同一通用旧攻击，也不得用加弹量、延长弹体寿命或叠加互不相关机制制造“丰富”。
- 单个波次或 Boss 段以一个主母题为中心，最多加入一个已学会的辅助压力；新机制必须先单独教学再组合。颜色、弹形和运动规则应保持稳定语义，追分顺序不得遮蔽或关闭普通生存路线。这些是运行录像与实际游玩的设计判断，不编写依赖精确弹数、固定帧号或源码文本的脆弱测试。
