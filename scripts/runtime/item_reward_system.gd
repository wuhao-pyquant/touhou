...
func apply_collection(item_type: String, game_manager_ref: Object, collection_y: float, top_collection_y: float) -> Dictionary:
	...
	match behavior:
		"bullet_spread", "bullet_linear", "bullet_homing":
			...
			score_delta = base_score if base_score > 0 else 120
			resource_delta["shared_power"] = int(game_manager_ref.shared_power) - before_power
		"power":
			var before_power: int = int(game_manager_ref.shared_power)
			game_manager_ref.add_power(1)
			score_delta = base_score if base_score > 0 else 10
			resource_delta["shared_power"] = int(game_manager_ref.shared_power) - before_power
		"point":
			var point_base: int = base_score if base_score > 0 else int(rules.get("point_base", 10))
		...
		"bomb_refill":
			...
			score_delta = base_score if base_score > 0 else 100
		...
		"life":
			...
			score_delta = base_score if base_score > 0 else 500
		...
		"night_festival_seal":
			...
			var seal_base: int = base_score if base_score > 0 else int(rules.get("night_festival_seal_base", 1000))
		...
		"full_power":
			...
			score_delta = base_score if base_score > 0 else 300
			resource_delta["shared_power"] = int(game_manager_ref.shared_power) - before_power
		_:
