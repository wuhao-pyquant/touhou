...
func _assert_clock_discriminator_and_scratch_performance() -> void:
	...
	var unavailable_live_runtime_id := int(unavailable.stage2_field_topology_runtime.get_instance_id())
	var unavailable_live_world_id := int(unavailable.bullet_world.get_instance_id())
	var unavailable_scratch_world_id := int(unavailable.stage2_field_scratch_bullet_world.get_instance_id())
	var unavailable_runtime_before: Dictionary = unavailable.stage2_field_topology_runtime.capture_snapshot()
	var unavailable_world_before: Dictionary = unavailable.capture_bullet_world_state()
	...
	var live_world_before_busy_copy: Dictionary = timed.capture_bullet_world_state()
	...
	var live_runtime_before_alias: Dictionary = timed.stage2_field_topology_runtime.capture_snapshot()
	...
	while timings_us.size() < 120:
		...
		for runtime in [timed.stage2_field_topology_runtime, timed.stage2_field_topology_scratch_runtime]:
			var owner_id := int(runtime.get_instance_id())
...
func _assert_contract_loading_and_first_construction() -> void:
	...
	var pre_construction_runtime_id := int(normal.stage2_field_topology_runtime.get_instance_id())
	var pre_construction_world_id := int(normal.bullet_world.get_instance_id())
	var pre_construction_runtime: Dictionary = normal.stage2_field_topology_runtime.capture_snapshot()
	var pre_construction_world: Dictionary = normal.capture_bullet_world_state()
...
func _assert_event_entry_rollback() -> void:
	...
	var live_runtime_id_before := int(rollback.stage2_field_topology_runtime.get_instance_id())
	var scratch_runtime_id_before := int(rollback.stage2_field_topology_scratch_runtime.get_instance_id())
	var live_world_id_before := int(rollback.bullet_world.get_instance_id())
	var scratch_world_id_before := int(rollback.stage2_field_scratch_bullet_world.get_instance_id())
