...
param(
    ...
    [string]$LogFile,
    # Opt in to an actual visible/windowed engine process.  The default remains
    # headless for ordinary test tickets; windowed runs use the same retained
    # identities, mutex, job object, timeout and process-tree cleanup path.
    [switch]$Windowed,
...
function New-ProtocolPump([IO.Stream]$Stream) {
    [pscustomobject]@{Stream=$Stream;Buffer=(New-Object byte[] 1024);Task=$null;Line=(New-Object 'System.Collections.Generic.List[byte]');Frames=(New-Object 'System.Collections.Generic.List[string]');Text=(New-Object Text.StringBuilder);Total=0;Complete=$false;Failure=$null}
}
function Start-ProtocolPump([object]$Pump,[long]$Deadline) {
    if((Get-Remaining $Deadline) -le 0){throw 'protocol pump bootstrap deadline'}
    try{$Pump.Task=$Pump.Stream.ReadAsync($Pump.Buffer,0,$Pump.Buffer.Length)}catch{$Pump.Failure=$_.Exception.Message;$Pump.Complete=$true}
}
function Update-ProtocolPump([object]$Pump,[long]$Deadline) {
    if($Pump.Complete -or $null -eq $Pump.Task -or -not $Pump.Task.IsCompleted){return}
    try {
        $count=$Pump.Task.GetAwaiter().GetResult()
        if($count -eq 0){if($Pump.Line.Count -ne 0){throw 'unterminated protocol frame'};$Pump.Complete=$true;return}
        for($i=0;$i -lt $count;$i++){
            $Pump.Total++;if($Pump.Total -gt $script:MaxDiagnostics){throw 'diagnostic stream exceeds cap'}
            $byte=$Pump.Buffer[$i]
            if($byte -eq 10){if($Pump.Line.Count -gt $script:MaxFrame){throw 'protocol frame exceeds cap'};$line=[Text.Encoding]::UTF8.GetString($Pump.Line.ToArray()).TrimEnd("`r");[void]$Pump.Frames.Add($line);[void]$Pump.Text.Append($line).Append("`n");$Pump.Line.Clear();continue}
            $Pump.Line.Add($byte);if($Pump.Line.Count -gt $script:MaxFrame){throw 'unterminated protocol frame exceeds cap'}
        }
...
function Stop-StartedWorker([Diagnostics.Process]$Process,[bool]$ForceFailure) {
    # One kill attempt and one fixed grace are the entire emergency protocol.
    $killFailed=$false;try{if($ForceFailure){throw 'forced worker kill failure'};$Process.Kill()}catch{$killFailed=$true}
    $exited=$false;try{$exited=$Process.WaitForExit(1000)}catch{$exited=$false}
    return (-not $killFailed -and $exited)
}
...
namespace Gtr {
 ...
 public static class Native {
  static Mutex HeldFaultMutex;
  const uint TH32CS_SNAPPROCESS=2, PROCESS_QUERY_LIMITED_INFORMATION=0x1000, PROCESS_TERMINATE=1, SYNCHRONIZE=0x100000;
  const uint CREATE_SUSPENDED=4, EXTENDED_STARTUPINFO_PRESENT=0x80000, INFINITE=0xffffffff, WAIT_OBJECT_0=0, WAIT_ABANDONED=0x80, WAIT_TIMEOUT=258;
  ...
  [DllImport("kernel32.dll",SetLastError=true,CharSet=CharSet.Unicode)] static extern IntPtr OpenMutexW(uint access,bool inherit,string name);
  ...
  public static void AssertAbi(){
   ...
   static void FailWatch(SupervisorWatch w){lock(w.Gate){w.Failed=true;if(w.Job!=IntPtr.Zero&&!TerminateJobObject(w.Job,9))Environment.FailFast("supervisor watcher containment failure",new Win32Exception(Marshal.GetLastWin32Error()));}Environment.Exit(7);}
    ...
    var thread=new Thread(()=>{
     try{
      ...
      while(true){
       ...
       if(fault=="worker-kill-failure"){Thread.Sleep(5000);return;}
       FailWatch(w);return;
      }
   ...
   static void AttachWatch(SupervisorWatch w,IntPtr job){CheckWatch(w);IntPtr duplicate=IntPtr.Zero;Need(DuplicateHandle(GetCurrentProcess(),job,GetCurrentProcess(),out duplicate,0,false,2),"DuplicateHandle watcher job");try{lock(w.Gate){if(w.Failed||w.Handle==IntPtr.Zero)throw new InvalidOperationException("supervisor watcher failure");if(w.Job!=IntPtr.Zero)throw new InvalidOperationException("watcher job already attached");w.Job=duplicate;duplicate=IntPtr.Zero;}CheckWatch(w);}finally{if(duplicate!=IntPtr.Zero)Need(CloseHandle(duplicate),"CloseHandle rejected watcher job");}}
   static void DetachWatch(SupervisorWatch w,IntPtr job){if(w==null)return;IntPtr duplicate=IntPtr.Zero;lock(w.Gate){duplicate=w.Job;w.Job=IntPtr.Zero;}if(duplicate!=IntPtr.Zero)Need(CloseHandle(duplicate),"CloseHandle detached watcher job");}
  ...
  public static Result Run(FileIdentity exe,string args,FileIdentity project,string outp,string errp,SupervisorWatch watch,long engineDeadline,long totalDeadline,int poll,string fault){
   ...
   try{
    ...
    if(attr!=IntPtr.Zero){DeleteProcThreadAttributeList(attr);Marshal.FreeHGlobal(attr);attr=IntPtr.Zero;}if(jobp!=IntPtr.Zero){Marshal.FreeHGlobal(jobp);jobp=IntPtr.Zero;}if(handles!=IntPtr.Zero){Marshal.FreeHGlobal(handles);handles=IntPtr.Zero;}
     CloseOrdered(ref oh,"CloseHandle stdout capture",ref cleanupError);CloseOrdered(ref eh,"CloseHandle stderr capture",ref cleanupError);CloseOrdered(ref pi.hProcess,"CloseHandle process",ref cleanupError);CloseOrdered(ref pi.hThread,"CloseHandle thread",ref cleanupError);if(fault=="pause-before-detach")Thread.Sleep(10000);DetachWatch(watch,j);CloseOrdered(ref j,"CloseHandle job",ref cleanupError);if(cleanupError!=null)throw new InvalidOperationException("native cleanup failure",cleanupError);
   }
  }
 }
}
...
function Invoke-Worker {
    ...
    $project='';$log='';$cleaned=@();$exit=$null;$category='InvalidInput';$status='failed';$exeIdentity=$null;$projectIdentity=$null;$watch=$null;$retainedMutex=[IntPtr]::Zero;$cleanupBegan=$false
    try {
        if($WorkerFault -eq 'empty-frame'){[Console]::Out.WriteLine('');[Console]::Out.Flush();return}
        ...
        if($WorkerFault -eq 'missing-ready'){[Console]::Out.WriteLine((New-Frame 'READY' $Nonce @{mutexKey=$key}));[Console]::Out.Flush();return}
        ...
        $go=Read-Frame (Read-ProtocolStdinLine) 'GO' $Nonce;Test-ExactProperties $go @('action') 'go';if($go.action -isnot [string] -or $go.action -cne 'go'){throw 'missing GO'}
        # The supervisor owns acquisition, while the worker retains a non-owning
        # handle before any inventory/mutation so the named object outlives a dead
        # supervisor long enough for the fail-closed contention path to observe it.
        $retainedMutex=[Gtr.Native]::RetainMutex($key)
        if($WorkerFault -eq 'worker-kill-failure'){[Gtr.Native]::AcquireRetainedMutexForFault($key)}
        if($WorkerFault -eq 'hang-after-go'){Start-Sleep -Seconds ($TimeoutSeconds+2)}
        if($WorkerFault -eq 'worker-kill-failure'){Start-Sleep -Seconds ($TimeoutSeconds+10)}
        ...
        if($WorkerFault -eq 'fractional-final'){[Console]::Out.WriteLine((New-Frame 'FINAL' $Nonce @{status='failed';category='ProcessFailure';processExitCode=1.5;project=$project;logPath=$log;cleanedPids=@();elapsedSeconds=0}));[Console]::Out.Flush();return}
        if($WorkerFault -eq 'cleaned-without-begin'){[Console]::Out.WriteLine((New-Frame 'FINAL' $Nonce @{status='failed';category='CleanupFailure';processExitCode=$null;project=$project;logPath=$log;cleanedPids=@(1);elapsedSeconds=0}));[Console]::Out.Flush();return}
        ...
        $engine=@($args);if(-not $Windowed){$engine+=@('--headless')};$engine+=@('--path',$project,'--log-file',$log); $line=(($engine|ForEach-Object{Quote-WindowsArgument $_}) -join ' ');if((Quote-WindowsArgument $exeIdentity.DiagnosticPath).Length+1+$line.Length -gt $script:MaxWindowsCommand){throw 'engine command exceeds cap'}
        $r=$null
        ...
        if($cleanupBegan){$category='CleanupFailure'}elseif($WorkerFault -in @('watcher-failure','watcher-ready-timeout')){$category='CleanupFailure'}
...
function Invoke-Supervisor {
    ...
    $category='InvalidInput';$status='failed';$exit=$null;$project='';$log='';$cleaned=@();$mutex=$null;$mutexHeld=$false;$worker=$null;$workerCreated=$false;$workerStarted=$false;$workerExitConfirmed=$false;$emergencyUsed=$false;$stdoutPump=$null;$stderrPump=$null;$hadReady=$false;$firstWorkerFrame='';$readyProject='';$cleanupCount=$null;$cleanupObserved=$false;$nextFrame=1
    try {
        ...
        $forward=@('-NoProfile','-ExecutionPolicy','Bypass','-File',$PSCommandPath,'-Worker','-GodotPath',$GodotPath,'-ProjectPath',$ProjectPath,'-TimeoutSeconds',$TimeoutSeconds,'-PollMilliseconds',$PollMilliseconds,'-Nonce',$nonce,'-SupervisorPid',$PID,'-SupervisorCreation',$creation,'-DeadlineTicks',$deadline);if($Windowed){$forward+='-Windowed'}
        ...
        $goBytes=[Text.Encoding]::ASCII.GetBytes((New-Frame 'GO' $nonce @{action='go'})+"`n");$goStream=$worker.StandardInput.BaseStream;$goStream.Write($goBytes,0,$goBytes.Length);$goStream.Flush();$goStream.Close();if($WorkerFault -eq 'worker-kill-failure'){Start-Sleep -Milliseconds 1000;$mutex.ReleaseMutex();$mutexHeld=$false}
        # S4/S5: drain and authenticate CLEANUP_BEGIN while the worker is live.
        # Once seen, every later timeout, loss, or uncertain termination is cleanup
        ...
        if($final.status -isnot [string] -or $final.category -isnot [string] -or $final.project -isnot [string] -or $final.logPath -isnot [string] -or $final.status -notin @('passed','failed') -or -not $script:Codes.ContainsKey($final.category) -or [string]::IsNullOrWhiteSpace($final.project) -or [string]::IsNullOrWhiteSpace($final.logPath) -or $final.project -cne $readyProject){throw 'invalid final contract'}
        if($final.elapsedSeconds -isnot [double] -and $final.elapsedSeconds -isnot [single] -and $final.elapsedSeconds -isnot [decimal] -and $final.elapsedSeconds -isnot [int] -and $final.elapsedSeconds -isnot [int64]){throw 'invalid final elapsed'};if([double]::IsNaN([double]$final.elapsedSeconds) -or [double]::IsInfinity([double]$final.elapsedSeconds) -or $final.elapsedSeconds -lt 0){throw 'invalid final elapsed'}
        $candidateExit=$null;if($null -ne $final.processExitCode){$candidateExit=Test-Integral $final.processExitCode 'final exit' 0 4294967295}
        if($final.cleanedPids -isnot [Collections.IEnumerable] -or $final.cleanedPids -is [string]){throw 'invalid cleaned pids'};$candidateCleaned=@();foreach($cleanedPid in @($final.cleanedPids)){$candidateCleaned+=Test-Integral $cleanedPid 'cleaned pid' 1 4294967295};if(@($candidateCleaned|Sort-Object -Unique).Count -ne $candidateCleaned.Count){throw 'duplicate cleaned pid'};if(-not $cleanupObserved -and $candidateCleaned.Count -ne 0){throw 'cleaned pids before cleanup'};if($cleanupObserved -and $candidateCleaned.Count -gt $cleanupCount){throw 'inconsistent cleaned pid count'};if($final.category -eq 'Success' -and ($final.status -ne 'passed' -or $candidateExit -ne 0)){throw 'inconsistent successful final'};if($final.category -ne 'Success' -and $final.status -ne 'failed'){throw 'inconsistent failed final'}
        $status=$final.status;$category=$final.category;$exit=$candidateExit;$project=$final.project;$log=$final.logPath;$cleaned=@($candidateCleaned)
    } catch {
        [Console]::Error.WriteLine("supervisor failure: $($_.Exception.Message)")
        if($cleanupObserved){$category='CleanupFailure'}elseif($WorkerFault -in @('watcher-failure','watcher-ready-timeout')){$category='CleanupFailure'}elseif($_.Exception.Message -match 'mutex'){$category='LockContention'}elseif($_.Exception.Message -match 'unconfirmed worker'){$category='CleanupFailure'}elseif($_.Exception.Message -match 'ready deadline|pump bootstrap deadline' -and (Get-Remaining $deadline) -le 0){$category='Timeout'}elseif($hadReady -and $_.Exception.Message -match 'protocol|contract|integral|worker ready|worker protocol|extra or missing|pump'){ $category='LaunchFailure'}elseif(-not $hadReady -and $firstWorkerFrame -notlike 'GTR1 FINAL *' -and ($firstWorkerFrame -like 'GTR1 READY*' -or $_.Exception.Message -match 'protocol|pump')){$category='LaunchFailure'}elseif($_.Exception.Message -match 'deadline'){$category='Timeout'}elseif($_.Exception.Message -match 'worker launch|Start'){$category='LaunchFailure'}elseif($category -notin @('CleanupFailure','PreexistingHeadlessProcess','FatalOutput','ProcessFailure','LaunchFailure')){$category='InvalidInput'}
    } finally {
        if($workerStarted -and -not $worker.HasExited -and -not $emergencyUsed){$emergencyUsed=$true;$workerExitConfirmed=Stop-StartedWorker $worker ($WorkerFault -eq 'worker-kill-failure');if(-not $workerExitConfirmed){$category='CleanupFailure'}}
        # An unconfirmed worker may still own descendants: abandon, never release, the mutex.
        if($mutex -and $workerExitConfirmed){if($mutexHeld){try{$mutex.ReleaseMutex()}catch{}};$mutex.Dispose()};$clock.Stop();$code=$script:Codes[$category];if($category -eq 'Success'){$status='passed';$code=0}else{$status='failed'};Write-Output (Write-Summary $status $category $exit $clock.Elapsed.TotalSeconds $project $log $cleaned);exit $code
    }
}
