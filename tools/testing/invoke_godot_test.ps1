...
$script:MaxFrame=16384; $script:MaxInput=65536; $script:MaxCapture=262144; $script:MaxDiagnostics=262144
...
function Test-PublicArguments {
    ...
    if($hasJson){try{$decoded=ConvertFrom-Json -InputObject $GodotArgumentJson -ErrorAction Stop}catch{throw 'GodotArgumentJson must be JSON'};if($null -eq $decoded -or $decoded -is [string] -or -not ($decoded -is [Collections.IEnumerable])){throw 'GodotArgumentJson must be a nonempty array of strings'};$all=@($decoded);if($all.Count -eq 0){throw 'GodotArgumentJson must be a nonempty array of strings'};foreach($item in $all){if($item -isnot [string] -or $item.Length -gt $script:MaxInput -or $item.IndexOf([char]0)-ge 0){throw 'GodotArgumentJson must be a bounded array of strings'}};return ,$all}
    foreach($item in @($GodotArgument)){if($item -isnot [string] -or $item.Length -gt $script:MaxInput -or $item.IndexOf([char]0)-ge 0){throw 'invalid GodotArgument'}}
    return ,@($GodotArgument)
}
...
namespace Gtr {
 ...
 public sealed class SupervisorWatch { public IntPtr Handle; public long Creation; public IntPtr Job; public volatile bool Failed; public readonly ManualResetEvent Stop=new ManualResetEvent(false); public readonly ManualResetEvent Done=new ManualResetEvent(false); }
 ...
 public static class Native {
  ...
  static void CheckWatch(SupervisorWatch w){if(w==null||w.Failed||w.Handle==IntPtr.Zero||Created(w.Handle)!=w.Creation)throw new InvalidOperationException("supervisor watcher failure");uint r=WaitForSingleObject(w.Handle,0);if(r==WAIT_TIMEOUT)return;if(r==0xffffffff)throw new Win32Exception(Marshal.GetLastWin32Error(),"WaitForSingleObject supervisor");throw new InvalidOperationException("supervisor watcher failure");}
  static void StartWatch(SupervisorWatch w,long deadline){new Thread(()=>{try{while(Now()<deadline){if(w.Stop.WaitOne(5))return;uint r=WaitForSingleObject(w.Handle,0);if(r==WAIT_TIMEOUT)continue;if(r==0xffffffff)Environment.FailFast("supervisor watcher wait failure",new Win32Exception(Marshal.GetLastWin32Error()));w.Failed=true;IntPtr j=w.Job;if(j!=IntPtr.Zero&&!TerminateJobObject(j,9))Environment.FailFast("supervisor watcher containment failure",new Win32Exception(Marshal.GetLastWin32Error()));Environment.Exit(7);}}finally{w.Done.Set();}}){IsBackground=true}.Start();}
  static void AttachWatch(SupervisorWatch w,IntPtr job){CheckWatch(w);w.Job=job;CheckWatch(w);}
  ...
  public static FileIdentity OpenIdentity(string path,bool directory){var sa=new SECURITY_ATTRIBUTES{nLength=Marshal.SizeOf(typeof(SECURITY_ATTRIBUTES)),bInheritHandle=false};IntPtr h=CreateFileW(path,0x80|SYNCHRONIZE,7,ref sa,3,directory?0x02000000u:0,IntPtr.Zero);if(h==INVALID)throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile identity");try{return FromHandle(h);}catch{Need(CloseHandle(h),"CloseHandle failed identity");throw;}}
  ...
  static Match SameProject(string cmd,FileIdentity project){var a=Argv(cmd);if(a==null||a.Length==0)return Match.Ambiguous;bool headless=false;int paths=0;for(int i=1;i<a.Length;i++){if(a[i]=="--")break;if(a[i]=="--headless"){if(headless)return Match.Ambiguous;headless=true;continue;}if(a[i].StartsWith("--headless=",StringComparison.OrdinalIgnoreCase))return Match.Ambiguous;string p=null;if(a[i]=="--path"){if(++i>=a.Length)return Match.Ambiguous;p=a[i];}else if(a[i].StartsWith("--path=",StringComparison.OrdinalIgnoreCase)){p=a[i].Substring(7);if(p.Length==0)return Match.Ambiguous;}if(p!=null){if(!Path.IsPathRooted(p)||++paths>1)return Match.Ambiguous;FileIdentity x=null;try{x=OpenIdentity(p,true);if(!x.Same(project))return Match.VerifiedOther;}catch{return Match.Ambiguous;}finally{if(x!=null)CloseIdentity(x);}}}return headless&&paths==1?Match.VerifiedTarget:Match.VerifiedOther;}
  ...
  static string FinishCapture(CaptureRead read,long deadline){long remaining=deadline-Now();if(remaining<=0||!read.Done.WaitOne((int)Math.Min(int.MaxValue,remaining)))throw new TimeoutException("capture deadline");if(read.Error!=null)throw new InvalidOperationException("capture read failed",read.Error);return Encoding.UTF8.GetString(read.Data??new byte[0]);}
  ...
  public static SupervisorWatch OpenSupervisorWatch(int pid,long creation,long deadline,string fault){IntPtr h=OpenProcess(SYNCHRONIZE|PROCESS_QUERY_LIMITED_INFORMATION,false,(uint)pid);if(h==IntPtr.Zero||fault=="watcher-failure"){if(h!=IntPtr.Zero)Need(CloseHandle(h),"CloseHandle rejected supervisor");throw new InvalidOperationException("supervisor watcher failure");}var w=new SupervisorWatch{Handle=h,Creation=creation};try{CheckWatch(w);StartWatch(w,deadline);CheckWatch(w);return w;}catch{Need(CloseHandle(h),"CloseHandle failed supervisor watch");w.Handle=IntPtr.Zero;throw;}}
  public static void VerifySupervisorWatch(SupervisorWatch w){CheckWatch(w);}
  public static void CloseSupervisorWatch(SupervisorWatch w,long deadline){if(w==null||w.Handle==IntPtr.Zero)return;w.Stop.Set();long remaining=deadline-Now();if(remaining<=0||!w.Done.WaitOne((int)Math.Min(int.MaxValue,remaining)))throw new TimeoutException("supervisor watcher close deadline");Need(CloseHandle(w.Handle),"CloseHandle supervisor");w.Handle=IntPtr.Zero;}
  ...
  public static Result Run(FileIdentity exe,string args,FileIdentity project,string outp,string errp,SupervisorWatch watch,long engineDeadline,long totalDeadline,int poll,string fault){
   ...
   try{
    ...
    if(fault=="pause-before-resume")Thread.Sleep(10000);CheckWatch(watch);if(fault=="resume-failure")throw new Win32Exception(5,"ResumeThread");if(ResumeThread(pi.hThread)==uint.MaxValue)throw new Win32Exception(Marshal.GetLastWin32Error(),"ResumeThread");
    var result=new Result();while(true){long now=Now();if(now>=engineDeadline){result.TimedOut=true;break;}uint w=WaitForSingleObject(pi.hProcess,(uint)Math.Min(poll,Math.Max(1,engineDeadline-now)));if(w==0xffffffff)throw new Win32Exception(Marshal.GetLastWin32Error(),"WaitForSingleObject engine");if(w==WAIT_TIMEOUT)continue;if(w!=WAIT_OBJECT_0)throw new InvalidOperationException("unexpected engine wait state");if(Empty(j,fault)){uint code;Need(GetExitCodeProcess(pi.hProcess,out code),"GetExitCodeProcess");result.ExitCode=(int)code;result.Empty=true;break;}}
    if(!result.Empty){Need(TerminateJobObject(j,2),"TerminateJobObject");while(Now()<totalDeadline&&!Empty(j,fault))Thread.Sleep((int)Math.Min(10,Math.Max(1,totalDeadline-Now())));result.Empty=Empty(j,fault);}
    ...
    if(made&&j!=IntPtr.Zero){try{if(!Empty(j,"")){Need(TerminateJobObject(j,2),"TerminateJobObject cleanup");while(Now()<totalDeadline&&!Empty(j,""))Thread.Sleep((int)Math.Min(10,Math.Max(1,totalDeadline-Now())));if(!Empty(j,""))throw new InvalidOperationException("job cleanup proof failed");}}catch(Exception e){cleanupError=e;}}
...
function Invoke-Worker {
    $clock=[Diagnostics.Stopwatch]::StartNew(); $budget=[Math]::Max(1,[int](($DeadlineTicks-[Diagnostics.Stopwatch]::GetTimestamp())*1000/[Diagnostics.Stopwatch]::Frequency))
    $project='';$log='';$cleaned=@();$exit=$null;$category='InvalidInput';$status='failed';$exeIdentity=$null;$projectIdentity=$null;$watch=$null;$deadlineMs=0
    try {
        ...
        if($WorkerArgumentB64){$args=@($WorkerArgumentB64.Split(',')|ForEach-Object{ConvertFrom-B64Url $_});if($args.Count -eq 0){throw 'provide exactly one argument form'}}else{$args=Test-PublicArguments}
        ...
        Initialize-WorkerNative; [Gtr.Native]::AssertAbi();$deadlineMs=[long]($DeadlineTicks*1000/[Diagnostics.Stopwatch]::Frequency);$watch=[Gtr.Native]::OpenSupervisorWatch($SupervisorPid,$SupervisorCreation,$deadlineMs,$WorkerFault);$exeIdentity=[Gtr.Native]::OpenIdentity($GodotPath,$false);$projectIdentity=[Gtr.Native]::OpenIdentity($ProjectPath,$true);$project=$projectIdentity.DiagnosticPath
        # W2: mutex derives from stable volume/file identity, not a canonical string.
        $key=Get-Sha256 "$($projectIdentity.Volume):$($projectIdentity.IndexHigh):$($projectIdentity.IndexLow)"; [Console]::Out.WriteLine((New-Frame 'READY' $Nonce @{mutexKey=$key;project=$project}));[Console]::Out.Flush()
        ...
        if($existing.Count){[Console]::Out.WriteLine((New-Frame 'CLEANUP_BEGIN' $Nonce @{count=$existing.Count}));[Console]::Out.Flush();$category='CleanupFailure';[Gtr.Native]::VerifySupervisorWatch($watch);$cleaned=@([Gtr.Native]::Cleanup($existing,$WorkerFault,$deadlineMs));[Gtr.Native]::VerifySupervisorWatch($watch);$again=@([Gtr.Native]::Inventory($exeIdentity,$projectIdentity,''));if($again.Count){throw 'cleanup verification failed'};$category='InvalidInput'}elseif($WorkerFault -eq 'retained-identity'){$category='CleanupFailure';throw 'retained identity changed'}
        ...
        $category='CleanupFailure';$r=[Gtr.Native]::Run($exeIdentity,$line,$projectIdentity,"$log.stdout","$log.stderr",$watch,([Diagnostics.Stopwatch]::GetTimestamp()*1000/[Diagnostics.Stopwatch]::Frequency+$engineRemaining),$deadlineMs,$PollMilliseconds,$WorkerFault);$category='InvalidInput'
        $exit=[int]$r.ExitCode
        ...
        if($WorkerFault -eq 'watcher-failure'){$category='CleanupFailure'}
...
function Invoke-Supervisor {
    ...
    try {
        ...
        $argumentB64=[string]::Join(',',@($workerArguments|ForEach-Object{ConvertTo-B64Url $_}));$forward+=@('-WorkerArgumentB64',$argumentB64);if($LogFile){$forward+=@('-LogFile',$LogFile)};if($CleanupExisting){$forward+='-CleanupExisting'};if($WorkerFault){$forward+=@('-WorkerFault',$WorkerFault)}
        ...
        while($stdoutPump.Count -lt 1 -and -not $stdoutPump.Complete -and (Get-Remaining $clock $budget) -gt 0){Start-Sleep -Milliseconds 5};if($stdoutPump.Failure){throw $stdoutPump.Failure};if($stdoutPump.Count -ne 1){throw 'deadline worker ready'};try{$ready=Read-Frame $stdoutPump.Frame(0) 'READY' $nonce}catch{Start-Sleep -Milliseconds 25;throw "$($_.Exception.Message) worker=$($stderrPump.Text)"};$project=[string]$ready.project
        ...
        # S4/S5: one fixed emergency grace follows kill; no later drain/wait can extend it.
        $remain=Get-Remaining $clock $budget;if($remain -le 0 -or -not $worker.WaitForExit($remain)){$worker.Kill();$workerExitConfirmed=$worker.WaitForExit(1000);if(-not $workerExitConfirmed){$category='CleanupFailure';throw 'unconfirmed worker termination'};throw 'deadline worker'};$workerExitConfirmed=$true
        while((-not $stdoutPump.Complete -or -not $stderrPump.Complete) -and (Get-Remaining $clock $budget) -gt 0){Start-Sleep -Milliseconds 5};if($stdoutPump.Failure){throw $stdoutPump.Failure};if($stderrPump.Failure){throw $stderrPump.Failure};if(-not $stdoutPump.Complete -or -not $stderrPump.Complete){throw 'deadline pipe completion'}
        # S6: READY [CLEANUP_BEGIN] FINAL is the complete authenticated protocol.
        $index=1;if($stdoutPump.Count -eq 3){$null=Read-Frame $stdoutPump.Frame(1) 'CLEANUP_BEGIN' $nonce;$index=2}elseif($stdoutPump.Count -ne 2){throw 'extra or missing worker protocol frame'};$final=Read-Frame $stdoutPump.Frame($index) 'FINAL' $nonce;[Console]::Error.Write($stderrPump.Text)
        ...
        if($status -notin @('passed','failed') -or -not $script:Codes.ContainsKey($category) -or $final.PSObject.Properties.Name -notcontains 'processExitCode' -or $final.PSObject.Properties.Name -notcontains 'project' -or $final.PSObject.Properties.Name -notcontains 'logPath' -or $final.PSObject.Properties.Name -notcontains 'cleanedPids'){throw 'invalid final contract'};if($category -eq 'Success' -and ($status -ne 'passed' -or $null -eq $exit)){throw 'inconsistent successful final'};if($category -ne 'Success' -and $status -ne 'failed'){throw 'inconsistent failed final'}
    } catch {
        [Console]::Error.WriteLine("supervisor failure: $($_.Exception.Message)")
        if($WorkerFault -eq 'watcher-failure'){$category='CleanupFailure'}elseif($_.Exception.Message -match 'mutex'){$category='LockContention'}elseif($_.Exception.Message -match 'deadline'){$category='Timeout'}elseif($_.Exception.Message -match 'worker launch'){$category='LaunchFailure'}elseif($category -notin @('CleanupFailure','PreexistingHeadlessProcess','FatalOutput','ProcessFailure','LaunchFailure')){$category='InvalidInput'}
    } finally {
        if($worker -and -not $worker.HasExited){try{$worker.Kill();$workerExitConfirmed=$worker.WaitForExit(1000)}catch{$workerExitConfirmed=$false};if(-not $workerExitConfirmed){$category='CleanupFailure'}}
